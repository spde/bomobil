...
function processObjectDeep(id){
	return function (returnData){
		...
		//Put data in object array
			...
			objects[id]['images'] = $(returnData).find("table[id=dlMultimedia] img").map(function(){pattern = /ico_pdf\.gif$/; alert("http://www.boplats.se"+$(this).attr("src").trim()+" - "+validateContentType("http://www.boplats.se"+$(this).attr("src").trim(), "image")); if (pattern.test($(this).attr("src")) == false && validateContentType("http://www.boplats.se"+$(this).attr("src").trim(), "image") == true){return "http://www.boplats.se"+$(this).attr("src").trim()}}).get();
